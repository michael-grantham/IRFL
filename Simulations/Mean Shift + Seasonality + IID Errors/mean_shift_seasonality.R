...
for(i in 1:20){
  ...
  # For the no-trend model with 11 seasonal params and 3 changepoints (2 params each): 11 + 6 = 17
  true_param_count <- 17
  true_bic = n * log(sum((mu + seasonal_effect - y)^2) / n) + true_param_count * log(n)
  
  out = c("301,601,901",3,true_bic,paste(B,collapse=","))    # Repeat 7 iteratively reweighted estimations
