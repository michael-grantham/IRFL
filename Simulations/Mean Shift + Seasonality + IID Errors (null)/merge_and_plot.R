...
knots = ggplot(df2, aes(x = Label, y = `Mean Shifts`, fill = Label)) +
  ...
  scale_y_continuous(breaks = c(3, seq(-1, 1, by = 1))) +
...
sim = ggplot(NULL, aes(x = time)) +
  ...
  geom_line(aes(y = 0), linewidth = .75, color = "red") +
  labs(x = "t", y = expression(y[t])) + 
...
ggsave("Seasonality (Null) Simulation Plot.pdf", plot = simplot, device = "pdf", width = 14, height = 3, dpi = 300)
ggsave("Seasonality (Null) Group Plot.pdf", plot = (sim + per + knots + seasonality ) + plot_layout(ncol = 2, guides = "collect") &
         theme(legend.position = "right",
               legend.justification = c("center"),
               legend.title = element_text(face = "bold", size = 12),
               legend.text = element_text(size = 10)),
       device = "pdf", width = 14, height = 6, dpi = 300)
ggsave("Seasonality (Null) Combined Plot.pdf", plot = g, device = "pdf", width = 14, height = 9, dpi = 300)
