...
write.csv(final, "final_out_null_neg0.7.csv", row.names = FALSE)
...
compare = phi_plot + nknots_plot + 
  ...
  plot_annotation(title = expression(phi==-0.7),
                  theme = theme(
                    plot.title = element_text(hjust = 0.5) # Center the title
                  ))


ggsave("Fixed Trend Simulations (null).pdf", 
       plot = sig, device = "pdf", width = 14, height = 6, dpi = 300)
ggsave("Fixed Trend Comparisons Null (phi=-0.7).pdf", 
