...
file_names = c("final_out_neg0.7.csv",
               "final_out_neg0.3.csv",
               "final_out_0.0.csv",
               "final_out_0.3.csv",
               "final_out_0.7.csv")
...
write.csv(out, "data_group_plot.csv")
...
phi_plot = data |> 
  mutate(`Ada FL` = phi_adaflasso,
        ...
        # `IRFL (YW)` = phi_yw_best_iter,
         WCM = phi_WCM,
         AR1Seg = phi_AR1seg) |> 
  pivot_longer(cols = 29:32,names_to = "Method",
               values_to = "phi_hat") |>
...
nknots_plot = data |> 
  ...
  ggplot(aes(x = Method, 
             y = nknots-true_nknots, fill = Method)) +
  geom_boxplot(position = position_dodge(width = 0.8)) + 
  scale_fill_manual(
    values = c("Ada FL" = "lightblue", 
               "IRFL" = "lightcoral",
...
# Plot of distance
distance_plot = data |> 
  mutate(`Ada FL` = distance_adaflasso,
         `IRFL` = distance_best_iter_flasso,
         WCM = distance_WCM,
         AR1Seg = distance_AR1seg) |> 
  pivot_longer(cols = 29:32,names_to = "Method",
               values_to = "distance") |>
  mutate(Method = factor(Method, levels = c(
    "Ada FL","IRFL",
...
# BIC plot (ΔBIC relative to true-model BIC), faceted by true_phi
bic_plot = data |>
  mutate(
    `Ada FL`        = bic_adaflasso,
    `IRFL`= min_bic_best_iter,
    WCM             = bic_WCM,
    AR1Seg          = bic_AR1seg
  ) |>
  pivot_longer(
    cols = c(`Ada FL`, `IRFL`, WCM, AR1Seg),
    names_to = "Method",
    values_to = "BIC"
  ) |>
  mutate(
    Method   = factor(Method, levels = c("Ada FL","IRFL","WCM","AR1Seg")),
    deltaBIC = BIC - bic_true_model
  ) |>
  ggplot(aes(x = Method, y = deltaBIC, fill = Method)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  scale_fill_manual(
    values = c(
      "Ada FL"         = "lightblue",
...
ggsave("AR1 Group BIC Plot.pdf",
       plot = bic_plot, device = "pdf", width = 14*.5, height = 6*.5, dpi = 300)
ggsave("AR1 Group Phi Plot.pdf", 
       plot = phi_plot, device = "pdf", width = 14*.5, height = 6*.5, dpi = 300)
ggsave("AR1 Group nknot Plot.pdf", 
       plot = nknots_plot, device = "pdf", width = 14*.5, height = 6*.5, dpi = 300)
ggsave("AR1 Group Distance Plot.pdf", 
       plot = distance_plot, device = "pdf", width = 14*.5, height = 6*.5, dpi = 300)
