...
semifinal = out |> 
  select(c("true_model","bic_true_model","true.season", "true.alpha",
           "flasso","bic_flasso","season_flasso","alpha_flasso",
           "adaflasso","bic_adaflasso","season_adaflasso","alpha_adaflasso")) |>
  mutate(best_iter = min_bic_iter,
         changepoints = min_bic_iter_flasso,
         min_bic_flasso = min_bic,
         min_bic_season = season_best_iter,
         min_bic_slope = slope_best_iter,
...
df = final %>% 
  pivot_longer(cols = 18:20, names_to = "method", values_to = "distance") %>%
...
#### Combine Plots
simplot = sim + per + plot_layout(ncol = 2)

g = (sim + per + knots + dist + seasonality + slope) +
...
ggsave("Seasonality Simulation Plot.pdf", plot = simplot, device = "pdf", width = 14, height = 3, dpi = 300)
ggsave("Seasonality Group Plot.pdf", plot = (knots + dist + seasonality + slope) + plot_layout(ncol = 2, guides = "collect") &
         theme(legend.position = "right",
               legend.justification = c("center"),
               legend.title = element_text(face = "bold", size = 12),
               legend.text = element_text(size = 10)),
       device = "pdf", width = 14, height = 6, dpi = 300)
ggsave("Seasonality Combined Plot.pdf", plot = g, device = "pdf", width = 14, height = 9, dpi = 300)
